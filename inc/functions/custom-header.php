...
function twentyseventeen_custom_header_setup() {
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => __( 'Default Header Image', 'applicator' ),
		),
	) );
}
...
endif; // End of applicator_header_style.

/**
 * Customize video play/pause button in the custom header.
 */
function twentyseventeen_video_controls( $settings ) {
	$settings['l10n']['play'] = '<span class="screen-reader-text">' . __( 'Play background video', 'applicator' ) . '</span>' . twentyseventeen_get_svg( array( 'icon' => 'play' ) );
	$settings['l10n']['pause'] = '<span class="screen-reader-text">' . __( 'Pause background video', 'applicator' ) . '</span>' . twentyseventeen_get_svg( array( 'icon' => 'pause' ) );
	return $settings;
}
