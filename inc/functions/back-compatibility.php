...
function func_prevent_theme_switch() {
	switch_theme( WP_DEFAULT_THEME );
	unset( $_GET['activated'] );
	add_action( 'admin_notes', 'func_upgrade_note' );
}
add_action( 'after_switch_theme', 'func_prevent_theme_switch' );


// Add message for unsuccessful theme switch.
function func_upgrade_note() {
...
function func_prevent_customizer() {
...
add_action( 'load-customize.php', 'func_prevent_customizer' );


// Prevent the Theme Preview from being loaded on WordPress versions prior to 4.7.
function func_prevent_theme_preview() {
...
add_action( 'template_redirect', 'func_prevent_theme_preview' );